...
如果 `<OPENCLAW_HOME>` 下缺少 `fleet/bin/taskbus.py` 或 `fleet/registry/`，就说明舰队尚未正确安装或初始化完成，此时王熙凤不得静默进入执行阶段。
...
main (王熙凤)
    ...
    |
    +-- pilot-general   (贾宝玉 / 通用编排)
    +-- pilot-research  (林黛玉 / 研究编排)
    +-- pilot-build     (贾探春 / 构建编排)
             |
             +-- worker-drive  (晴雯 / 实现与交付)
             +-- worker-guard  (袭人 / 测试与校验)
             +-- worker-sense  (小红 / 检索与分析)
```
...
| 层级 | Agent ID | 展示角色 | 责任 |
|------|----------|----------|------|
| 控制平面 | `main` | 王熙凤 | 接收用户请求、创建任务、选择编排代理、跟踪状态、汇总输出 |
| 任务编排 | `pilot-general` | 贾宝玉 | 处理混合型或边界不清的任务拆解 |
| 任务编排 | `pilot-research` | 林黛玉 | 处理研究、比较、资料收集、分析类任务 |
| 任务编排 | `pilot-build` | 贾探春 | 处理实现、修改、构建、验证闭环类任务 |
| 执行层 | `worker-drive` | 晴雯 | 负责实现、编辑、构建、交付具体结果 |
| 执行层 | `worker-guard` | 袭人 | 负责审查、测试、验证、约束检查 |
| 执行层 | `worker-sense` | 小红 | 负责搜索、抓取、阅读、分析信息 |
...
在任何编排代理被拉起之前，`main` 会先通过 [`fleet/bin/taskbus.py`](fleet/bin/taskbus.py) 把任务写入文件化控制平面。
...
对于新任务，`taskbus.py create` 是唯一合法入口。只有它成功返回之后，才允许拉起编排代理。
...
编排代理接收到的是显式任务上下文，而不是隐式继承的聊天记忆。

### 4. 执行代理执行

如果编排代理需要并行子任务，它可以按需拉起执行代理：

- `worker-drive`，也就是晴雯，负责实现
- `worker-guard`，也就是袭人，负责审查和验证
- `worker-sense`，也就是小红，负责检索和分析

执行代理是叶子节点，只返回聚焦结果，不拥有任务主状态。

### 5. 结果汇总

编排代理将执行代理的结果汇总成任务级结论，再返回给 `main`。  
任务主状态仍然只由 `main` 更新。
...
只有 `main` 能推进任务主状态。编排代理可以建议状态变化，但不直接控制全局注册表。
...
- 最多 3 个活动编排代理，作为编排策略
- 每个编排代理最多 2 个执行代理，作为编排策略
- 执行代理超时为 900 秒，来自 OpenClaw sub-agent 默认配置
...
2. `pilot-build` 拉起 `worker-drive`，也就是晴雯，完成功能实现
3. `pilot-build` 拉起 `worker-guard`，也就是袭人，做审查与验证
4. 编排代理汇总结果并返回结构化报告
5. `main` 更新任务状态并向用户汇报
...
2. `pilot-research` 拉起 `worker-sense`，也就是小红，收集、比较、分析信息
3. 如有需要，再由 `worker-guard`，也就是袭人，做交叉验证
4. 编排代理产出带权衡的结构化建议
5. `main` 记录结论和下一步
...
2. 每个任务拥有自己的编排代理 session 和状态文件
3. 切换焦点时不会覆盖另一个任务的上下文
4. 如果某个编排代理 session 失效，`main` 可以从 `HANDOFF.md` 与 `STATUS.json` 恢复
